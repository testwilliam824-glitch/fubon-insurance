...
function generateRecommendations(answers) {
  ...
  if (answers.age === '0-18') {
    recommendations.push({
      ...
      category: 'medical',
      title: '兒少基礎保障組合',
      products: ['金來寶小額終身壽險', '保幼平安傷害保險', '金享福住院醫療定額終身健康保險附約'],
    ...
    if (answers.income === 'below_50' || answers.budget === 'below_3000') {
      recommendations.push({
        ...
        category: 'life',
        title: '入門高保障方案',
        products: ['安享定期壽險', '優世代平準型定期壽險', '通勤安心網路投保傷害保險'],
        reason: '年輕時以低保費建立高保障',
      ...
      recommendations.push({
        ...
        category: 'saving',
        title: '青年理財儲蓄方案',
        products: ['珍給利利率變動型增額終身壽險', '新平準終身壽險', '吉利好運利率變動型增額終身壽險'],
        reason: '兼顧保障與資產累積',
    ...
    recommendations.push({
      ...
      category: 'life',
      title: '家庭支柱保障方案',
      products: ['富享世代利率變動型終身壽險', '金享福保本終身健康保險', '真康泰終身健康保險'],
      reason: '家庭主要收入者必備保障',
    ...
    if (answers.income === '200_500' || answers.income === 'above_500') {
      recommendations.push({
        ...
        category: 'saving',
        title: '資產增值方案',
        products: ['月月紅富分紅終身保險', '吉鑽利利率變動型還本終身保險', '樂活年年終身保險'],
        reason: '穩定現金流 + 資產增值',
    ...
    recommendations.push({
      ...
      category: 'retirement',
      title: '退休準備方案',
      products: ['金滿富利利率變動型年金保險(甲型)', '吉鑽利利率變動型還本終身保險', '豐盛年年利率變動型終身保險'],
      reason: '準備退休收入與長照規劃',
    ...
    recommendations.push({
      ...
      category: 'ltc',
      title: '長照與傳承方案',
      products: ['樂鑫安長期照顧終身健康保險', '和馨長期照顧終身健康保險附約', '鑫安e生即期年金保險'],
      reason: '退休期醫療保障與資產傳承',
  ...
  if (isHighAsset) {
    if (answers.tax_need === 'yes_urgent' || answers.tax_need === 'yes_consider') {
      recommendations.unshift({
        ...
        category: 'tax',
        title: '⭐ 高資產節稅傳承方案（優先）',
        products: ['新吉好利利率變動型增額終身壽險', '珍給利利率變動型增額終身壽險', '富好鑽利率變動型增額終身壽險'],
    ...
    if (answers.inheritance === 'yes_plan' || answers.inheritance === 'yes_consider') {
      recommendations.push({
        ...
        category: 'inheritance',
        title: '多世代傳承方案',
        products: ['金來寶小額終身壽險', '富享世代利率變動型終身壽險', '富利傳家利率變動型終身壽險'],
  ...
  if (
    ...
    !recommendations.some((r) => r.products.some((p) => /外幣/.test(p)))
  ) {
    recommendations.push({
      priority: 'low',
      category: 'foreign',
      title: '外幣資產配置方案',
      products: ['美利大運外幣利率變動型終身壽險', '星美富足外幣分紅終身壽險', '紅富美好外幣分紅終身壽險'],
  ...
  return {
    ...
    disclaimer:
      '本系統推薦僅為初步參考，實際保費、保額、給付條件與適用條款須由富邦人壽合格保險經紀員根據您完整的財務狀況、健康告知與需求面談後規劃。最終商品內容以保單條款為準。',
  };
}
