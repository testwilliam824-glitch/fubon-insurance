...
```bash
cd /Users/william/Desktop/富邦保單智能推薦系統
git init
git add .
git commit -m "Initial commit: 富邦保單智能推薦系統"
```

然後到 GitHub 建立新 repo（例如 `fubon-insurance`），再執行：

```bash
git remote add origin https://github.com/<你的帳號>/fubon-insurance.git
git branch -M main
...
4. 進入服務頁面 → 「**Environment**」分頁，補上：
   ...
   - `PUBLIC_URL`（**部署完成後**填回你的 Render 網址，例如 `https://fubon-insurance.onrender.com`）

### 方法 B：手動建立 Web Service

1. 「**New +**」→「**Web Service**」→ 連結 GitHub repo
2. 設定：
   - **Name**：`fubon-insurance`
   - **Region**：Singapore（最近）
   - **Branch**：`main`
...
部署完成後，Render 會給你類似：
```
https://fubon-insurance.onrender.com
```
...
4. 到 LINE Developers → Messaging API → Webhook URL：
   ```
   https://fubon-insurance.onrender.com/api/line/webhook
   ```
